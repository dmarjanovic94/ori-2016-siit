...
class State(object):
    ...
    def __init__(self, board, parent=None, position=None, goal_position=None):
        ...
        self.picked_blue_boxes = copy.deepcopy(parent.picked_blue_boxes) if parent is not None else []
        self.picked_orange_boxes = copy.deepcopy(parent.picked_orange_boxes) if parent is not None else []
...
class RobotState(State):
    def __init__(self, board, parent=None, position=None, goal_position=None):
        ...
        if parent is not None:
            self.picked_blue_boxes = copy.deepcopy(parent.picked_blue_boxes)
            self.picked_orange_boxes = copy.deepcopy(parent.picked_orange_boxes)
        else:
            self.picked_blue_boxes = []
            self.picked_orange_boxes = []

        # Ako smo trenutno na polju koje je kutija dodamo kutiju
        if self.position in board.blue_boxes and self.position not in self.picked_blue_boxes:
            self.picked_blue_boxes.append(self.position)
        
        # Uopsteno
        # if self.position in board.orange_boxes and self.position not in self.picked_orange_boxes:
        # Lak zadatak B
        # if len(self.picked_blue_boxes) == 1 and self.position in board.orange_boxes and self.position not in self.picked_orange_boxes:
        # Tezak zadatak A
        if len(self.picked_blue_boxes) > len(self.picked_orange_boxes) and self.position in board.orange_boxes and self.position not in self.picked_orange_boxes:
            self.picked_orange_boxes.append(self.position)
        
        # Ako je broj kutija jednak broju svih kutija ili roditelj ima sve kutije
        #self.has_boxes = (len(self.picked_blue_boxes) == len(board.boxes)) or (parent is not None and parent.has_boxes)
        
        # Lak zadatak A
        # self.goal_condition = len(self.picked_blue_boxes) == 1
        # Lak zadatak B
        # self.goal_condition = len(self.picked_blue_boxes) == 1 and len(self.picked_orange_boxes) == 1
        # Srednji zadatak A
        # self.goal_condition = len(self.picked_blue_boxes) == 3
        # Srednji zadatak B
    ...
    def get_legal_positions(self):
        ...
        # Lak zadatak A
        '''
        if len(self.picked_blue_boxes) == 0:
            d_rows = [0, 0, 1, -1]
            d_cols = [1, -1, 0, 0]
        '''
    ...
    def is_final_state(self):
        ...
        return self.position == self.goal_position and self.goal_condition

    def unique_hash(self):
        # Izmijenjen unique_hash zbog stanja
        # Dodat i spisak kutija koje je pokupio.
        # Ne moze samo broj - bitno je koje je kutije pokupio.
        return str(self.position) + str(self.picked_blue_boxes) + str(self.picked_orange_boxes)
